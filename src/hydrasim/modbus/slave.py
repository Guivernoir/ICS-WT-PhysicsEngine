...
from .maintenance_bridge import ModbusMaintenanceMixin
...
class ModbusServerConfig:
    ...
    vendor_name: str = "HydraSim"
    product_code: str = "HS-1000"
    vendor_url: str = "https://github.com/Guivernoir/HydraSim"
    product_name: str = "HydraSim Process Simulator"
...
class ModbusSlave(ModbusMaintenanceMixin):
    """
    ...
    @staticmethod
    def _read_block_values(
        block: ModbusSequentialDataBlock, address: int, count: int
    ) -> list[int]:
        values = block.getValues(address, count)
        if not isinstance(values, list):
            raise ValueError("Modbus data block read failed")
    ...
    def read_holding_register(self, name: str) -> float:
        ...
        try:
            ...
            with self._lock:
                if reg.data_type == "float32":
                    values = self._read_block_values(self.hr_block, wire_address, 2)
                    return self.decoder.registers_to_float32(values[0], values[1])
                elif reg.data_type == "int16":
                    values = self._read_block_values(self.hr_block, wire_address, 1)
                    return self.decoder.register_to_int16(values[0])
                elif reg.data_type == "uint16":
                    values = self._read_block_values(self.hr_block, wire_address, 1)
                    return self.decoder.register_to_uint16(values[0])
                raise ValueError(f"Unsupported holding register type: {reg.data_type}")
    ...
    def read_coil(self, name: str) -> bool:
        ...
        try:
            ...
            with self._lock:
                values = self._read_block_values(self.co_block, wire_address, 1)
